...
export type InventoryRow = {
  ...
  priceEurFoil?: string;
  priceTix?: string;
  auditHistory?: { id: string; changeType: string; reason?: string; createdAt: string }[];
};
...
function CardDetail({ row, onClose, isAdmin, onEdit }: { row: InventoryRow; onClose: () => void; isAdmin: boolean; onEdit: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 bg-black/50" onClick={onClose}><div className="absolute right-0 top-0 h-full w-full max-w-2xl overflow-y-auto bg-zinc-950 border-l border-zinc-800 p-4" onClick={(e)=>e.stopPropagation()}>
    <div className="flex items-start justify-between mb-4"><h2 className="text-xl font-bold">{row.cardName}</h2><div className="flex gap-2">{isAdmin ? <button onClick={onEdit} className="border px-2">Edit Inventory Item</button> : null}<button onClick={onClose} className="border px-2">Close</button></div></div>
    <div className="grid md:grid-cols-[240px_1fr] gap-4"><div className="rounded border border-zinc-800 bg-zinc-900 p-2">{getCardImage(row)?<img src={getCardImage(row)} alt={row.cardName} className="w-full rounded"/>:<div className="aspect-[63/88] flex items-center justify-center text-sm text-zinc-400">No image</div>}</div>
      <div className="space-y-2 text-sm">
        ...
        <div className="pt-2 border-t border-zinc-800"><p className="font-semibold mb-1">History</p>{row.auditHistory?.length ? row.auditHistory.map(h => <div key={h.id} className="text-xs text-zinc-300">{new Date(h.createdAt).toLocaleString()} • {h.changeType}{h.reason ? ` • ${h.reason}` : ''}</div>) : <div className="text-xs text-zinc-500">No manual edit history yet.</div>}</div>
      </div></div></div></div>;
}

export function InventoryBrowser({ rows, players, rounds, isAdmin, onSaveEdit, onSearchPrintings }: { rows: InventoryRow[]; players: PickRef[]; rounds: PickRef[]; isAdmin: boolean; onSaveEdit: (formData: FormData) => Promise<void>; onSearchPrintings: (formData: FormData) => Promise<ScryfallResult[]>; }) {
  ...
  return <div className="space-y-3">
    ...
    {selected ? <CardDetail row={selected} onClose={() => setSelected(null)} isAdmin={isAdmin} onEdit={() => { setEditing(selected); setSelected(null); }} /> : null}
